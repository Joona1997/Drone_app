...
describe("getDistance", () => {
    ...
    it("return return the distance between two points", () => {
        expect((0, DroneViolations_1.getDistance)(Y, X)).toBe(100);
    });
});
describe("parseXML", () => {
    ...
    it("parses xml string, check for timestamp", () => {
        expect((0, DroneViolations_1.parseXML)(xmlData).clock.snapshotTimestamp).toBe("2022-12-21T07:16:47.252Z");
    });
    it("parses xml string, check for drones serialNumber", () => {
        expect((0, DroneViolations_1.parseXML)(xmlData).drone.serialNumber).toBe("SN-GATxXVN4UI");
    });
    it("parses xml string, check for drones positionY", () => {
        expect((0, DroneViolations_1.parseXML)(xmlData).drone.positionY).toBe(489185.40421627177);
    });
    it("parses xml string, check for drones positionX", () => {
        expect((0, DroneViolations_1.parseXML)(xmlData).drone.positionX).toBe(204597.55685130172);
    });
});
describe("getUser", () => {
